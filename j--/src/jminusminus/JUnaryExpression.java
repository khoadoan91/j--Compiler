...
/**
 * The AST node for a unary bitwise not (~) expression.
 */

class JBitwiseNotOp extends JUnaryExpression {

    /**
     * Construct an AST node for a bitwise not expression given its line number,
     * and the operand.
     * 
     * @param line
     *            line in which the negation expression occurs in the source
    ...
    /**
     * Analyzing the bitwise not operation involves analyzing its operand, checking
     * its type and determining the result type.
     * 
     * @param context
     *            context in which names are resolved.
    ...
    public void codegen(CLEmitter output) {
        arg.codegen(output);
        output.addNoArgInstruction(INEG);	
        // FIXME find a corrent JVM instruction for bitwise not operation
    }
